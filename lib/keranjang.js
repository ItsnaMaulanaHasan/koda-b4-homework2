...
const hapusPesanan = async () => {
  ...
  while (loop) {
    ...
    if (indexkeranjang === 98) {
      ...
      const confirm = String(await input(` Yakin ingin menghapus pesanan ${item.nama} (y/n)? `, "text")).toLowerCase();
      if (confirm === "y") {
        keranjang = keranjang.splice(indexkeranjang, 1);
